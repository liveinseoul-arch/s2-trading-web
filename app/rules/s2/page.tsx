...
const RULES: { t: string; d: string }[] = [
  ...
  { t: "추가매수", d: "직전 매수가 −10%마다 추가매수, 최대 3차. 1차와 동일 금액(정액). 단 2차 매수 후 3차 매수 가격이 직전 최저가보다 낮을 때는 신저가 손절 감시와 충돌하므로 감시주문에서 제외 (broker 우선 체결 모호성 회피)." },
  { t: "분할매도", d: "평단 +3% / +5% / +7%에서 10% / 10% / 80% 매도." },
  { t: "손절", d: "분할매도 한 단계가 체결되면 그 단계가를 손절가로 상향. 2차 매수 후엔 신저가 손절(직전 최저가 하향 시 종가 청산)." },
  { t: "기간 손절", d: "진입 후 15영업일(약 3주) 안에 분할매도 1차도 못 찍으면 그 종가에 강제 청산. 화석 포지션 누적 차단 + 자본 회전 ↑ (12년 백테스트 Calmar 0.19→0.90)." },
    ...
    <>
      ...
      <Section title="검증 성과 (정직 공개)">
        <ul className="flex flex-col gap-2 text-sm">
          <li><b>11.8년(2014-08~2026-06, 현재 유니버스, 무비용 모델 · 시초 매도·추가매수일 매도 보류 · 3주 기간 손절 적용)</b>:
            CAGR <b className="text-accent">~9.7%</b> · MDD <b className="text-accent">~−10.8%</b> ·
            Calmar <b className="text-accent">~0.90</b> · 승률 ~87%.</li>
          <li className="text-muted">기간 손절 미적용 시(이전 버전): CAGR ~5.7% · MDD ~−31% · Calmar ~0.18. 화석 포지션 정리가 2018·2020 큰 폭락 손실의 핵심 해소 요인.</li>
