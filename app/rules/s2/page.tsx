      ...
      <Section title="파라미터 검증 (2026-07 실측 · 비용 0.215% 반영)">
        ...
        <ul className="mt-3 flex flex-col gap-2 text-sm">
          ...
          <li><b>왜 +2/+6/+14 를 안 쓰나</b> — 일봉상 16.05% 로 더 높지만 <b>그 우위는 &ldquo;터치=체결&rdquo; 가정에 의존</b>:
            ...
            +7% 는 확실히 뚫어 체결돼 <b>실현 가능성이 높다</b>.</span></li>
          <li><b>★2026-08-09 — 120일선 <u>위</u> 진입분만 +3/+6.5/+10 으로 확대(채택)</b>
            <span className="text-muted"> 아래 진입분(전체의 약 65%)은 +3/+5/+7 그대로.
            <b> 1차 +3% 를 양쪽 다 유지한 것이 핵심</b>이다 — 분할매도 한 단계가 체결돼야 손절이 이익 구간으로
            올라오므로, 1차를 +3.5% 로 올리면 손절 무장이 늦어져 <b>MDD 가 1.1%p 악화</b>하고
            1차 체결 실현율도 <b>91.7% → 79.1%</b> 로 떨어졌다.
            1차를 그대로 두고 2·3차만 넓히면 이미 이익 확정 손절이 걸린 뒤라 하방 위험이 늘지 않는다.
            <b> 분봉 실측에서 3차 +10% 의 실현율이 +7% 보다 오히려 높았다(82.9% → 92.0%)</b> —
            목표가 위 체류 시간 30분 → 200분, 참여율 0.353% → 0.145%. 강한 추세일 때만 +10% 에 닿기 때문이다(선택효과).
            12년 dry-run: CAGR 12.51 → <b>13.13%</b> · MDD −13.43 → <b>−13.38%</b>(개선) ·
            최종 NAV 4.10억 → <b>4.38억</b>.
            <b> 단 블록 부트스트랩은 &ldquo;구별 불가&rdquo;</b>(P(Δ&gt;0) 0.92–0.97) — 채택 근거는 통계적 유의성이 아니라
            위 구조와 개선의 광범위함(374후보 중 양 창 개선 228)이다.
            위/아래는 <b>진입 시점에 고정</b>되고 이후 바뀌지 않는다(실측 노출 1.9% · 3차 목표에서는 0%).</span></li>
