...
const RULES: { t: string; d: string }[] = [
  ...
  },
  {
    ...
    d: "한국: 주가 ₩3,000 이상 / 주간 거래대금 500억 이상 / 52주 고가 -30% 이내. 미국: 주가 $5 이상 / 주간 거래대금 $200M 이상 / 52주 고가 -30% 이내. 일본: 주가 ¥300 이상 / 주간 거래대금 200억엔 이상 / 52주 고가 -30% 이내. 상장 52주 미만 종목 제외. 이 필터는 RS 백분위 계산에는 쓰지 않고 매수 대상 선정에만 쓴다 — 필터 탈락 종목은 RS가 96 이상이어도 시장별 목록 페이지에 표시하지 않는다(주차별 판정: 과거 주차에 통과했으면 그 주차 목록에는 남음).",
  },
  {
    t: "한국(KR) 시총 필터",
