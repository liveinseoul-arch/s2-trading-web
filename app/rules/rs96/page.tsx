...
const BUY_RULES: { t: string; d: string }[] = [
  {
    t: "① 진입 신호 — RS 96+ & 시장 M-필터 (한국: half 모드)",
    d: "주간 RS가 96 이상으로 '신규' 진입한 종목만 매수 대상. 여기에 시장상태(M-필터)를 함께 건다: 지수(KOSPI/S&P500/N225)의 분산일(distribution day) 누적과 FTD(Follow-Through Day, 반등 4일차+ 1.7%↑ & 거래량 증가)로 시장을 ON/OFF 판정. 운용 방식은 두 가지 — 게이트(OFF 구간 신규 매수 전면 금지)와 half(OFF 구간에는 절반 비중으로만 진입). 2026-07 검증에서 한국은 half 모드가 우위: 2017~26 기준 CAGR 19.7% vs 게이트 14.5%, Sharpe 0.59 vs 0.49, MDD 동등(−48% 수준) — 국면 전환 직후 돌파 종목(예: 2023-02 에코프로비엠)을 게이트가 놓치는 기회비용을 절반 비중이 회수한다. 과거 2015~26 창에서도 같은 방향이 재현되어 한국 규칙은 half 모드를 채택 후보로 한다. 보유 포지션은 M 상태와 무관하게 매도 규칙만 적용(2023 에코프로 +658%는 OFF 구간 내내 보유하며 실현).",
  },
  {
    t: "② 체결 시점 — 다음 주 시가",
