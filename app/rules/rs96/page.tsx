    ...
    <>
      ...
      <Section title="재무(C) 필터 검증 — 미국·한국 상반된 결론 (2026-07)">
        ...
        <ul className="ml-4 list-disc text-sm leading-relaxed text-muted">
          ...
          <li>
            <b>한국 — 영업이익 기준은 유효 (검증 중)</b>: 같은 게이트를 <b>영업이익</b>으로 바꾸면
            결과가 반전된다. 영업이익 배증(C≥100%) 게이트는 2017~2026에서 CAGR 14.0%로 기준선
            (14.5%)과 대등하면서 <b>MDD는 절반 이하(−21% vs −47%), Sharpe 0.72로 기준선(0.49)과
            KOSPI(0.58)를 모두 상회</b>, 거래는 1/5로 줄었다 — 미국과 동일한 구조의 재현. 순이익의
            일회성 노이즈를 제거하면 한국에서도 &ldquo;진짜 이익 폭발&rdquo;은 알파라는 뜻이다.
            단 이 결과는 아직 자체 walk-forward 검증 전이라 정식 규칙 채택은 보류 상태다.
          </li>
        </ul>
        <p className="mt-2 text-sm font-medium leading-relaxed text-accent">
          ...
          역신호이므로 적용하지 않고, 영업이익 기준 게이트는 walk-forward 검증 통과 시 채택을
          검토한다.
        </p>
      </Section>
