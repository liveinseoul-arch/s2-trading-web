...
const RULES: { t: string; d: string }[] = [
  {
    ...
    d: "백분위 0~99. 매 주차에 광의 유동주 전체(미국: 주간 거래대금 $50M 이상 ≈ 백테스트 유니버스 정합, 한국·일본: 수집 유니버스 전체)의 52주 가중 모멘텀(composite return) 분포를 만들어, 그 종목이 분포의 몇 백분위인지를 등급화. RS 96 = 상위 4%. IBD/Minervini 의 표준 정의와 동치. 백분위 모집단을 백테스트와 일치시켰으며 잔차(±2~3pt)는 상장폐지 종목 포함 여부 차이(백테스트는 포함, 웹은 현존 상장만)에서 발생.",
  },
  {
  ...
  {
    t: "유니버스 필터 (매수 대상 = 목록 노출 조건)",
    d: "한국: 주가 ₩3,000 이상 / 주간 거래대금 500억 이상 / 52주 고가 -30% 이내 / 상장 52주 이상. 미국: 주가 $5 이상 / 주간 거래대금 $200M 이상 / 52주 고가 -30% 이내 / 상장 52주 이상. 일본: 주간 거래대금 ¥5억 이상만(≈백테스트 진입 유니버스 일 ¥1억) — 52주 고가·주가·상장기간 조건은 일본에서 전략 성과를 절반으로 깎아 적용하지 않는다(2026-07 분해 검증: 52주고가 조건만으로 CAGR 22.2→11.7%). 이 필터는 RS 백분위 계산에는 쓰지 않고 매수 대상 선정에만 쓴다 — 필터 탈락 종목과 거래정지 종목은 RS가 96 이상이어도 시장별 목록 페이지에 표시하지 않는다(주차별 판정: 과거 주차에 통과했으면 그 주차 목록에는 남음). 미국·한국 백테스트(성과 페이지)도 동일 필터를 진입 조건에 포함해 실매매 규칙과 정합(미국 10.6%/−22.0% 동등, 한국 14.9%/−21.2% 개선).",
  },
  {
    t: "한국(KR) 시총 필터",
...
const BUY_RULES: { t: string; d: string }[] = [
  {
    t: "① 진입 신호 — RS 96+ & 시장 M-필터 (시장별 상이: 미국 off · 한국 half · 일본 gate)",
    d: "주간 RS가 96 이상으로 '신규' 진입한 종목만 매수 대상. 여기에 시장상태(M-필터)를 건다: 지수(KOSPI/S&P500/N225)의 분산일(distribution day) 누적과 FTD(Follow-Through Day, 반등 4일차+ 1.7%↑ & 거래량 증가)로 시장을 ON/OFF 판정. 운용 강도는 3단계 — off(판정 무시, 항상 매수) / half(OFF 구간 절반 비중) / gate(OFF 구간 신규 금지). 2026-07 3시장 검증 결론은 시장마다 다르다. ① 미국 = off: gate는 파괴적(2002~26 CAGR −0.3% vs off 4.4%), half도 열위(3.9%) — 분산일·FTD 신호가 현대 미국의 짧은 V자 조정에 후행해 OFF 과판정(주간의 65%)을 낳는다. ② 한국 = half: 2017~26 CAGR 19.7% vs gate 14.5%(Sharpe 0.59 vs 0.49, MDD 동등), 2015~26 창에서도 재현. 지수가 약세여도 테마 주도주가 독립적으로 달리는 시장(예: 2023-02 에코프로비엠)이라 완전 차단은 기회비용이 크다. ③ 일본 = gate: 2017~26 CAGR 22.2%/Sharpe 0.86 vs off 14.4%/0.63, half는 최악(10.8%). 시기 분할 재현 — 약세 구간(17~21) +7.5%p 압승, 강세 구간(22~26) −3.4%p 소폭 지불 — 전형적 보험형 필터. 원리: M필터의 최적 강도는 '지수가 개별 주도주를 얼마나 대표하는가'(동조성)에 비례한다 — 일본(높음)은 이분법, 한국(낮음)은 절반, 미국(신호 후행)은 꺼짐. 보유 포지션은 모든 모드에서 M 상태와 무관하게 매도 규칙만 적용(2023 에코프로 +658%는 OFF 구간 내내 보유하며 실현).",
  },
  {
    t: "② 체결 시점 — 다음 주 시가",
...
const SELL_RULES: { t: string; d: string }[] = [
  ...
  },
  {
    ...
    d: "기본 모드: 보유 중 고점 수익(peak gain)이 +20%에 도달하면 21일 EMA 종가 이탈 트레일이 켜지고, +50%→50일 EMA, +100%→75일 EMA, +200%→100일 EMA 로 점점 더 느슨한 트레일로 승격된다(4단). 초고수익 종목일수록 되돌림을 넓게 허용해 대시세를 끝까지 태운다. 한 번 승격되면 되돌아가지 않으며, −8% 손절은 단계와 무관하게 항상 병행 활성. (HOLDTIME 토글 시 보유 20일/50일 초과가 OR 조건으로 추가되고 EMA 단계에서 −8%가 해제되지만 기본 OFF.) 아래 '매도 트리거 · 활성 조건' 표 참조.",
  },
  {
    t: "④ 상장폐지 강제 청산",
...
const SELL_STAGES = [
  ...
  },
  {
    ...
    note: "변동성을 견디며 대시세를 추적. 승격 후 미복귀",
  },
  {
    stage: "④ 75EMA 트레일",
    arm: "peak gain ≥ +100% 도달 이후 (③을 대체)",
    trigger: "일봉 종가가 75일 EMA 이하",
    note: "2배 이상 승자는 더 느슨하게 태운다",
  },
  {
    stage: "⑤ 100EMA 트레일",
    arm: "peak gain ≥ +200% 도달 이후 (④를 대체)",
    trigger: "일봉 종가가 100일 EMA 이하",
    note: "3배 이상 초대형 승자의 되돌림 최대 허용",
  },
  {
    stage: "⑥ RS 이탈",
    arm: "항상 활성",
            ...
            <tbody>
              <tr className="border-b border-[var(--color-borderc)] last:border-0 align-top">
                ...
                <td className="px-2 py-1.5">
                  ...
                  CAGR 10.6%/MDD −22.0%/Sharpe 0.84 (지수 13.0%/−31.8%/0.80). 성과(US) 페이지는
                  이 결합안 + 진입 유니버스 4-필터(실매매 매수 규칙 정합)의 최근 10년 백테스트로 작성.
                </td>
