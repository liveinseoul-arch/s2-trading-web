...
// ★★「진입 0건」 대각선을 ★몇 건부터 그을까 — ★못 산 것이 ★놀라운 날에만 긋는다.
//   ⚠️★문턱 없이 그었더니 T1 격자의 ★82.5%가 대각선이 됐다(2026-08-23 실측) = ★배경이 된다.
//   ★실측 — 후보가 많을수록 「못 샀다」가 드물어진다(양 시스템 동일):
//        후보 1건  S2 77.0% · T1 93.5%  ← ★못 사는 게 기본값. 정보 없음
//        후보 2건  S2 57.8% · T1 78.1%
//        후보 3–4  S2 37.1% · T1 66.2%
//        후보 9/10+ S2 11.1% · T1 20.0%  ← ★여기서 못 샀으면 놀랍다
//   ★3건으로 두면 S2 30일(1.1%) · T1 133일(5.6%) — 신호 밀도가 적절하다.
const DIAG_MIN = 3;
              ...
              {Array.from({ length: cols }, (_, col) => (
                <div key={col} className="flex flex-col gap-0.5">
                  {Array.from({ length: 5 }, (_, row) => {
                    ...
                    const noEntry = c.n >= DIAG_MIN && !ret[c.date]?.n;
