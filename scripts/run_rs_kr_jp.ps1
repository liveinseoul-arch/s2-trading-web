...
# ★★★[2026-09-14 · CAND-2026-09-14-4 · ★해달별님 승인] 단계별 치명도 분리 ★개방.
#   ⚠️★실측 사고(2026-09-12 01:06) — 7단계 테마 분류가 Gemini 429(monthly spending cap)로
#     exit=1 을 내자 ★잡 전체가 [ABORT] 로 중단됐다. ★앞 단계는 전부 성공했는데(ETF 2,632건 적재)
#     ★테마 라벨만 못 붙이고 ★잡이 죽었다. ⚠️주간 잡이라 ★다음 실행 09-18 = ★6일 공백.
#   ★이 줄이 켜지면 ★`-NonFatal` 이 붙은 단계(현재 ★7 classify KR ★하나뿐)는
#     ★실패해도 ★알림만 내보내고 ★잡을 계속한다. ★수집·계산 단계는 ★그대로 치명이다.
#   ⚠️★★2026-08-16 「전체 패키지」를 ★뒤집는 것이 아니다 — ★그 규칙의 표적은
#     ★「데이터가 안 들어왔는데 ★조용히 끝나는 것」이고 ★7단계는 ★라벨링이다.
#     ★★알림은 ★그대로 나간다(★침묵하지 않는다).
#   ★★되돌리기 — ★이 한 줄을 지운다(기본 미설정 = ★종전 비트동일).
#   근거: quant_infra/2026-09/SUMMARY_2026-09-14.md §①1-b · 커밋 61aea70
$env:RS_STAGE_NONFATAL = "1"
...
#   ⚠️★★[2026-09-18 정정 · 해달별님 지적] ★캡을 낮춰
#     ★「₩20,000 초과면 flash」를 구현하려던 ★B 안은 ★틀렸다 — ★철회.
#   ★근거 — ★지출 캡 429 는 ★모델별이 아니라 ★★프로젝트 단위다:
#     'Your project has exceeded its monthly spending cap.'  (2026-09-12 실측 로그)
#     ★즉 캡에 닿으면 ★flash 도 ★똑같이 막힌다 — ★캡을 낮추면
#     ★더 빨리 ★탈출구 없이 죽는다.
#   ⚠️★★따라서 ★아래 FALLBACK 은 ★지출 캡에는 ★도움이 되지 않는다 —
#     ★모델별 한도(RPM·TPM) 429 에만 유효하다. ★그만큼은 이득이라 ★남겨 둔다.
#   ★「₩20,000 초과면 flash」는 ★하드 캡 ★전에 전환해야 하므로
#     ★월 소진액을 ★아는 장치가 필요하다 — ★미구현(결정 대기).
