...
#   CP949 로 오독하면서, 「끝」(EB 81 9D)처럼 잔여 선두바이트로 끝나는 주석이 뒤 LF 를 페어로 삼켜
#   다음 줄 $env:BT_MARKET = "2" 가 통째로 주석에 흡수됐다(2026-08-16 수동 실행 실측 — 엔진이
#   대화형 「시장 선택」 프롬프트로 떨어져 EOFError exit 1). CRLF 시절엔 CR 이 희생돼 무사고였다.
#   BOM 이 있으면 PS 5.1 이 UTF-8 로 정독한다(줄 삼킴·로그 모지바케·커밋메시지 깨짐 동시 해소).
