...
# ★★★[2026-09-02 신설 · CAND-2026-09-02-17 · 동결 스냅샷 전용] 진입 자격 거래대금 문턱.
#   [게이트] `S2_TV_MIN` — ★원 단위. ★미지정/빈문자열/0 = off = `cfg.min_trading_value_krw`
#     (= 500_000_000_000 = 5,000억) = ★종전 비트 동일.
#   ★왜 필요한가 — 문턱을 바꾸는 env 가 ★기존에 하나도 없다(run_eod.ps1 env 35 중 0개).
#     유일한 인접 게이트 `S2_TV_ADAPTIVE`(backtest.py:135)는 ★배율 축이고 ★운영 경로에서는
#     사문이다(`export_eod.py:1206` 이 `px["tv_thr"]` 를 안 보고 정액을 직접 쓴다).
#   ★되돌리기 — env 를 지운다.
_tvmin_s = os.environ.get("S2_TV_MIN", "").strip()
TV_MIN = float(_tvmin_s) if _tvmin_s else 0.0
# ★자격 통과 카운터(§4-2d 관문2) — 사문 판별용. 판정 무변경.
QUAL_N = {"tkday": 0, "qual": 0, "thr_won": 0.0}
...
def load(cfg: Config, end: date, start: date | None = None):
    ...
    thr = TV_MIN if TV_MIN > 0 else cfg.min_trading_value_krw
    QUAL_N["thr_won"] = float(thr)
    print(f"[TVMIN] 진입 자격 거래대금 문턱 = {thr:,.0f}원 ({thr/1e8:,.0f}억)"
          f"  창 WINDOW={WINDOW}거래일  "
          + ("★env S2_TV_MIN 지정" if TV_MIN > 0 else "off(=cfg 기본 5,000억 · 종전 비트동일)"),
          flush=True)
    sm, smy = {}, {}
    ...
    print(f"[TVMIN] 자격 통과 종목-일 = {QUAL_N['qual']:,} / 전체 {QUAL_N['tkday']:,}"
          f"  ({QUAL_N['qual']/max(QUAL_N['tkday'],1)*100:.4f}%)", flush=True)
